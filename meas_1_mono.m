...
for j = 1 : length(dat)
 ...
 if exist(sprintf("%s/%d.txt", src_path, dat(j,1)), "file") != 2; continue;
 elseif dat(j,1) < 1000; continue;
 end;
 ...
 diffs(i, k) = 100 * (var_e(k) - var_m) / var_m;

 end

 mn_diff = mean(diffs(i,:));
 u_diff = get_uncertainty(diffs(i,:));
