...
x = load('case_1_sine_3000_64.txt');
...
title("(a)\\rm {\\it{}f_{o}} = 3 kHz, {\\it{}N} = 64")
...
x = load('case_1_sine_9000_64.txt');

f_diff = 48e3/64;
f_vec = 9000 + x(:,1) * f_diff;
d_vec = x(:,2);
e_vec = x(:,3);
...
xlim([min(f_vec)-0.1*f_diff max(f_vec)+0.1*f_diff])
title("(b)\\rm {\\it{}f_{o}} = 9 kHz, {\\it{}N} = 64")
xlabel("Signal frequency, Hz")
ylabel("Error Value, %")
set_format(gca, 'Title', false);
set_format(gca, 'XY', false);
...
x = load('case_1_sine_3000_128.txt');
...
xlim([min(f_vec)-0.1*f_diff max(f_vec)+0.1*f_diff])
title("(c)\\rm {\\it{}f_{o}} = 3 kHz, {\\it{}N} = 128")
xlabel("Signal frequency, Hz")
ylabel("Error Value, %")
set_format(gca, 'Title', false);
set_format(gca, 'XY', false);
...
x = load('case_1_sine_9000_128.txt');
...
xlim([min(f_vec)-0.1*f_diff max(f_vec)+0.1*f_diff])
title("(d)\\rm {\\it{}f_{o}} = 9 kHz, {\\it{}N} = 128")
xlabel("Signal frequency, Hz")
ylabel("Error Value, %")
set_format(gca, 'Title', false);
set_format(gca, 'XY', false);
